...
await expect(page).aiAssert(`Assert stock news graph and bottom line are visible`);
await page.evaluate(({ deltaX, deltaY }) => {
  window.scrollBy(deltaX, deltaY);
}, { deltaX: 0, deltaY: 9000 });
await expect(page).aiAssert(`Related stocks are on the page and visible`);
await page.evaluate(({ deltaX, deltaY }) => {
  window.scrollBy(deltaX, deltaY);
}, { deltaX: 0, deltaY: -600 });
await expect(page).aiAssert(`Clickable cards are visible for collections that AAPL is a part of.`);});
});
