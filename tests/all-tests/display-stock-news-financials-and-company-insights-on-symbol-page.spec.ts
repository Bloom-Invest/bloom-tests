...
await aiAssert(page, `The AAPL symbol page is loaded showing the stock price and a chart with time period selectors (1D, 1W, 1M, etc.)`, { timeout: 60000, fullPage: true });
await page.evaluate(({ deltaX, deltaY }) => {
  window.scrollBy(deltaX, deltaY);
}, { deltaX: 0, deltaY: 9000 });
await page.waitForTimeout(2000);
await aiAssert(page, `Related stocks are on the page and visible`, { timeout: 60000, fullPage: true });
await page.evaluate(({ deltaX, deltaY }) => {
  window.scrollBy(deltaX, deltaY);
}, { deltaX: 0, deltaY: -600 });
await page.waitForTimeout(2000);
await aiAssert(page, `Clickable cards are visible for collections that AAPL is a part of.`, { timeout: 60000, fullPage: true });});
});
