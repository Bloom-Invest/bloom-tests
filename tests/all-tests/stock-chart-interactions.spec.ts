...
test("Stock chart allows switching time periods", async ({ page }) => {
  ...
  await test.step("Verify chart time period buttons are visible", async () => {
    ...
    for (const period of timePeriods) {
      const btn = page.locator('button, [role="button"]').filter({ hasText: new RegExp(`^${period}$`) }).first().describe(`${period} time period button`);
  ...
  await test.step("Switch time periods and verify chart updates", async () => {
    // Click 1Y
    await page.locator('button, [role="button"]').filter({ hasText: /^1Y$/ }).first().describe('1Y button').click();
    await page.waitForTimeout(1000);

    // Click 1D
    await page.locator('button, [role="button"]').filter({ hasText: /^1D$/ }).first().describe('1D button').click();
    await page.waitForTimeout(1000);
