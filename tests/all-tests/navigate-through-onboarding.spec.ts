...
await expect(page).aiAssert(`Assert that the chatbot answered the question`, { timeout: 60000 });
await page.getByRole('button', { name: 'Continue' }).describe('\'Continue\' button').click();});
...
await expect(page).aiAssert(`The user is on onboarding and can configure multiple notification types on or off.`, { timeout: 60000 });
...
await page.getByRole('button', { name: 'Continue' }).describe('\'Continue\' button').click();
await page.getByText('Notify me before trial ends').describe('\'Notify me before trial ends\' text label').click();
await page.getByRole('button', { name: 'Cancel' }).describe('\'Cancel\' button').click();
...
await expect(page).aiAssert(`Assert the user is on the chat tab`, { timeout: 60000 });
await page.getByRole('link', { name: 'Portfolio' }).describe('Portfolios link with briefcase icon').click({"timeout":9000});
await expect(page).aiAssert(`Assert AAPL and CPNG are on the watchlist`, { timeout: 60000 });});
});
